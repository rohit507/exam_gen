...
def create_gen(*vargs):
    """
    Creates a new rng from a seed string
    """
    return Random(create_seed(*vargs))
...
# # @attr.s
# # class UserRNG(UserSetup):

# #     rng_source = attr.ib(default=None, init=False)
# #     user_setup_rng = attr.ib(default=None, init=False)

# #     def __pre_user_setup__(self):

# #         super().__pre_user_setup__()

# #         if self.rng_source == None:
# #             raise RuntimeError("RNGSource not initialized.")

# #         user_data = self.classroom.get_data(self.student_id)
